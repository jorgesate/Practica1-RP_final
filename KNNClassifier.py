...
class KNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, kmax=50, kmin=2, kstep=1):
        self.neigbours = 0
        self.kmax = kmax
        self.kmin = kmin
        self.kstep = kstep
    ...
    def fit(self, X, y):
        ...
        for k in enumerate(self.ks):
            ...
            score_k = cross_val_score(KNNclsf, X, y, cv=5)
            self.scores[k] = score_k.mean()
            print(str(k))

        n_max = np.argmax(self.scores)
        k_best = self.ks[n_max]
        self.KNNclsf_best = KNeighborsClassifier(n_neighbors=k_best).fit(X, y)
